...
const OPEN_WEEK = JOBS.filter(
  (j) => Date.now() - new Date(j.datePosted).getTime() < 7 * 86_400_000
).length;
const TAGS = new Set(JOBS.map((j) => companyTag(j.companyName))).size;
      ...
      <section className="border-b border-border">
        <div className="mx-auto max-w-7xl px-4 pt-10 pb-8 sm:px-6">
          ...
          <div className="mt-6 flex flex-wrap gap-2">
            {[
              { value: `${TOTAL_ROLES}+`, label: "open roles" },
              { value: `${TOTAL_COMPANIES}`, label: "companies" },
              { value: `${TAGS}`, label: "company types" },
              { value: `${OPEN_WEEK}`, label: "posted this week" },
